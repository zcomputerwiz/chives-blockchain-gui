...
import { AlertDialog, Card, Flex, chivesToMojo, mojoToChivesLocaleString } from '@chives/core';
import {
  send_transaction,
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  ...
                  {mojoToChivesLocaleString(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToChivesLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = chivesToMojo(amount_input.value);
    const fee = chivesToMojo(fee_input.value);
